...
{
  电容列表:="6.2,4.1,4,3.7,3.4,3.3,2.9,2.6,2.3,2.55,2.1,1.8,1.65,1.5,1.2,1,0.8,0.68,0.65,0.5,0.47,0.33,0.3,0.25,0.24,0.22,0.1,0.068,0.047,0.033,0.027,0.022,0.01"
...
输出:="输入你需要生成的目标uf电容(无需单位)`n计算结果会自动复制到剪贴板`n当前电容列表个数：" . 长度 . "`n当前电容列表内容：`n"
...
loop %长度%
{
  ...
  loop %长度%
  {
    等效值:=(第一*列表[A_Index])/(第一+列表[A_Index])
    误差:=(输入-等效值)/输入*100
